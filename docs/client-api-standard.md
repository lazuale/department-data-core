...
Операция API должна решать одну законченную задачу и иметь однозначный набор параметров.
...
Если процесс требует учета исполнителя, API получает устойчивый идентификатор пользователя или рабочей роли. Отображаемое имя не используется как ключ.
...
Рабочему клиенту выдаются минимальные права на предусмотренные объекты `api`. Он не должен иметь произвольного `UPDATE` внутренних таблиц или права менять структуру базы.
...
Отдельная система версий API в первой реализации не обязательна. Достаточно не менять существующий контракт без причины и оформлять несовместимое изменение явно.
