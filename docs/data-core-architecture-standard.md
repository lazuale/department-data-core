...
Данные источника сначала сохраняются в `raw`. В `stg` приводятся типы и технические форматы. `xref` разрешает внешнюю идентичность. Только после этого данные включаются в каноническую модель `core`.
...
- `source` — система или место получения данных;
- `dataset` — конкретный структурированный набор внутри источника;
- `load` — отдельный факт загрузки набора.
...
Данные разных источников хранятся раздельно до момента их осмысленного сопоставления. `raw` не исправляется вручную ради получения желаемого результата на следующих слоях.
