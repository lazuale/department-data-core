...
|---|---|---|
| `meta` | источники, наборы данных, загрузки, сведения об их обработке и технические метаданные связанных файловых артефактов | дублировать бизнес-данные |
...
| `xref` | соответствие внешних ключей источников внутренним ID | хранить независимую копию сущности |
...
`meta` должна различать как минимум четыре понятия:
...
- `processing` — применение определенной версии правил к уже полученному входу.
...
Файловое поступление может содержать один или несколько файловых артефактов, связанных с `load`. `meta` хранит технические сведения, необходимые для идентификации, поиска и проверки целостности этих объектов.

`load` остается идентичностью поступления независимо от количества связанных файлов.

Транспорт определяет способ доставки и не подменяет идентичность источника или набора. Получение входа и его обработка различаются. Сведения об обработке относятся к `meta`; исполняющий обработку серверный код находится в `etl`.
...
### A04. Внешние ключи источников не становятся внутренней идентичностью
...
### A08. Файловый артефакт относится к загрузке

Файловый артефакт является физическим представлением файловой поставки и связывается с соответствующим `load`.

Состояние загрузки хранится в `meta`. Структурированное представление входа хранится в `raw`. Файловое хранилище содержит физический оригинал поставки.

После фиксации сохраненный оригинал остается неизменным. Исправленная поставка регистрируется как новое поступление.
...
Файловое поступление может дополнительно сохранять один или несколько физических артефактов, связанных с соответствующим `load`.
