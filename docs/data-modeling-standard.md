...
Структурированные связи не хранятся строками вида `12,15,18` и не раскладываются по колонкам `item_1`, `item_2`, `item_3`.
...
Внутренний ID используется во внутренних связях и не зависит от внешней системы.
...
Бизнес-ключ используется для идентификации и контроля уникальности в предметной области. Он не заменяет внутренний ID. Если устойчивого естественного бизнес-ключа нет, искусственный бизнес-ключ не придумывается.

Каждая самостоятельная таблица имеет первичный ключ. Составной первичный ключ уместен для чистой таблицы связи, если самой строке не требуется отдельная идентичность.
